#!/usr/bin/env python3
"""
Test script for the Alibaba Product Updater Web Application
"""
...
def test_webapp():
    ...
    print("🧪 Testing Alibaba Product Updater Web Application")
    print("=" * 60)
...
8718696176313,150,300"""
    ...
    # Test 4: Test price update start
    print("\n4. Testing price update start...")
    try:
        update_data = {
            'filename': filename,
            'update_type': 'price'
        ...
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                print("   ✅ Price update started successfully")
                job_id = data['job_id']
                print(f"   🆔 Job ID: {job_id}")
            else:
                print(f"   ❌ Price update start failed: {data.get('error')}")
                return False
        else:
